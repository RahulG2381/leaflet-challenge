...
function createFeatures(earthquakeData) {
  ...
  function markerSize(magnitude) {
    return magnitude * 30000;  // Adjust this factor based on your requirements
  }
  ...
  for (let i = 0; i < earthquakeData.length; i++) {
    ...
    earthquakeMarkers.push(
      ...
      }).bindPopup(`<h3>Place: ${earthquakeData[i].properties.place}</h3><hr><h3>Magnitude: ${earthquakeData[i].geometry.coordinates[2]}</h3><hr><p>${new Date(earthquakeData[i].properties.time)}</p>`)
    );
  }
